...
public:
  ...
  Edge() : p1(), p2(), ray(false) {}
  Edge(T x1, T y1, T x2, T y2, bool ray = false) : p1(Point<T>(x1, y1)), p2(Point<T>(x2, y2)), ray(ray) {}
  Edge(const Point<T>& p1, const Point<T>& p2, bool ray = false) : p1(p1), p2(p2), ray(ray) {}
  ...
  double dist(const Edge<T>& other){
    /*
  ...
  Edge extend(const std::vector< Point<T> >& polygon){
